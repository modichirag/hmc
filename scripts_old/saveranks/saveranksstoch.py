...
posname = 'stoch_voltality-hmctnuts'
print(posname)
if olong == 3: fdir = '/mnt/ceph/users/cmodi/hmc/outputs_long%d/%s/'%(olong, posname)
else: fdir = '/mnt/ceph/users/cmodi/hmc/outputs_long/%s/'%(posname)
refdraws = np.load(  '/mnt/ceph/users/cmodi/hmc/outputs_long/%s/default/samples.npy'%posname)
refdraws2d = refdraws.reshape(-1, refdraws.shape[-1])
ndim = refdraws.shape[-1]
print(ndim)
print(refdraws.shape)

if rank == 0:
    with open('/mnt/ceph/users/cmodi/hmc/outputs_long/%s/params.json'%posname) as ff:
        for ll in ff.readlines(): print(ll)
with open('/mnt/ceph/users/cmodi/hmc/outputs_long/%s/params.json'%posname, 'r') as fp:
...
def get_ess(x, xref):
    if x.shape[-1] != xref.shape[-1]:
        print('Dropping dimensions in ref to match x with shape ', x.shape)
        xref = xref[..., :x.shape[-1]].copy()
...
def combine(fpath):
    print("Combining for fpath %s"%fpath)
    for ftype in ['samples', 'counts', 'accepts', 'accepted']:
        try: 
            ss = []
            for i in range(50):
...
for istep in range(nsteps):    
    ...
    try:
        ...
        if os.path.isfile(fpath + '/samples.npy'): 
            f = np.load(fpath + '/samples.npy')[:, :, :ndim]
            #if olong == 3 and f.shape[0] != 12000: 
            #    print('Redo because shape is %d'%f.shape[0])
            #    combine(fpath)
            #else: print('File found at %s'%fpath)
        elif os.path.isfile(fpath + '/samples/00.npy'): 
                combine(fpath)
        elif os.path.isfile(fpath + '/samples/0.npy'): 
                combine(fpath)
        #read
        ranksamples = np.load(fpath + '/samples.npy')[::args.subsample, :, :ndim]
        drop = np.zeros(ranksamples.shape[1]).astype(bool)
        for i in range(ndim):
            mu, std = abs(refdraws2d[..., i].mean()), refdraws2d[..., i].std()
            drop += (abs(ranksamples[..., i].mean(axis=0)) < 0.1*mu)
            drop += (abs(ranksamples[..., i].mean(axis=0)) > 10*mu)
            drop += (ranksamples[..., i].std(axis=0) < 0.1*std)
            drop += (ranksamples[..., i].std(axis=0) > 10*std)
        ranksamples = ranksamples[:, ~drop]
        print('Loaded files ' , rank, ranksamples.shape)
        ...
        try:
            ...
            #rankevals = rankevals[:, ~drop]
            nevals = rankevals.sum(axis=(0, 1))[:2].sum()
        except:
            #nevals = (Nleapfrog + 3)*ranksamples.shape[1]*ranksamples.shape[0] #np.load(fpath + '/counts.npy')
            nevals = (Nleapfrog + 3)*50*ranksamples.shape[0] #np.load(fpath + '/counts.npy')
...
if rank == 0:
    ...
    ndim = errchain['step10'].shape[-1]
    names = ['dim%d'%d for d in range(ndim)]
