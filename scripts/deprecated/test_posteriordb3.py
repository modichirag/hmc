...
fdir0= '/mnt/ceph/users/cmodi/hmc/outputs_long/%s-def//'%posname
fdir= '/mnt/ceph/users/cmodi/hmc/outputs_long3/%s-def//'%posname
...
try:
    with open(fdir0 + 'params.json', 'r') as fp:
        todump = json.load(fp)
...
for ss in [1, 2, 5, 0.5]:
    ...
    step_size, Nleapfrog =  stepsizefid*ss, max(2, int(Nleapfrogfid/ss))
