...
for istep in range(nsteps):    
    ...
    if rank == 0:
        ...
        with open('./data/Ndim%02d/l%02d_chisqs.json'%(ndim, lpath), 'w') as fp:
            json.dump(chisqs, fp, sort_keys=True, indent=4)
        with open('./data/Ndim%02d/l%02d_chisqs.json'%(ndim, lpath), 'w') as fp:
            json.dump(chisqs, fp, sort_keys=True, indent=4)
        with open('./data/Ndim%02d/l%02d_costs.json'%(ndim, lpath), 'w') as fp:
        ...
        for ic in range(ndim):
            ...
            if ic == 0: 
                fpname = './data/Ndim%02d/l%02d_counts_sigma.json'%(ndim, lpath)
            else: fpname = './data/Ndim%02d/l%02d_counts_alpha%d.json'%(ndim, lpath, ic)
